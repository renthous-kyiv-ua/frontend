...
const HouseReg = () => {
  ...
  const handleFileUpload = (e, type, index) => {
    ...
    // Проверка формата файлов и размеров
    if (type === 'photo' && !['image/jpeg', 'image/jpg', 'image/png'].includes(file.type)) {
      alert('Формат файла должен быть jpg, jpeg или png');
      return;
    }
    if (type === 'video' && file.type !== 'video/mp4') {
      alert('Формат видео должен быть mp4');
      return;
    }

    const maxSize = type === 'photo' ? 4 * 1024 * 1024 : 50 * 1024 * 1024;
    if (file.size > maxSize) {
      alert(`Размер ${type === 'photo' ? 'фото' : 'видео'} не должен превышать ${type === 'photo' ? '4' : '50'} MB`);
      return;
    }
  ...
  const renderButtonContent = (type, index) => {
    const previewContent = preview[`${type}${index}`];
    if (type === 'photo') {
      return previewContent ? (
        <img src={previewContent} alt={`Preview ${index}`} />
      ) : (
        <svg width="128" height="172" viewBox="0 0 128 172" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect width="128" height="128" rx="30" fill="#C2C1C1" fillOpacity="0.75" />
        ...
        <div className="additional-filling">
          <div className="class-home">
            ...
            <div className="button-group">
              <button className="class-button" onClick={() => setFormData({ ...formData, class: 'Luxury' })}>Luxury</button>
              <button className="class-button" onClick={() => setFormData({ ...formData, class: 'Comfort' })}>Comfort</button>
              <button className="class-button" onClick={() => setFormData({ ...formData, class: 'Standart' })}>Standart</button>
            </div>
          </div>
            ...
            <div className="button-group">
              {[...Array(6).keys()].map((room) => (
                <button
                  ...
                  key={room}
                  onClick={() => setFormData({ ...formData, rooms: (room + 1).toString() })}
                >
                  {room + 1}
                </button>
              ))}
              <button className="room-button" onClick={() => setFormData({ ...formData, rooms: '7+' })}>7+</button>
            </div>
          </div>
          <div className="class-walls">
            <label>Wall material</label>
            <div className="button-group walls">
              <button className="wall-button" onClick={() => setFormData({ ...formData, wallMaterial: 'Ceramic brick' })}>Ceramic brick</button>
              <button className="wall-button" onClick={() => setFormData({ ...formData, wallMaterial: 'Ceramic hollow brick' })}>Ceramic hollow brick</button>
              <button className="wall-button" onClick={() => setFormData({ ...formData, wallMaterial: 'Ceramic block' })}>Ceramic block</button>
              <button className="wall-button" onClick={() => setFormData({ ...formData, wallMaterial: 'Sand-lime brick' })}>Sand-lime brick</button>
              <button className="wall-button" onClick={() => setFormData({ ...formData, wallMaterial: 'Foam block' })}>Foam block</button>
              <button className="wall-button" onClick={() => setFormData({ ...formData, wallMaterial: 'Reinforced concrete panel' })}>Reinforced concrete panel</button>
            </div>
            <button className="more-walls">More</button>
          </div>
          ...
          <div className="class-heating">
            <label>Heating</label>
            <div className="button-group">
              <button className="heating-button" onClick={() => setFormData({ ...formData, heating: 'Centralized' })}>Centralized</button>
              <button className="heating-button" onClick={() => setFormData({ ...formData, heating: 'Individual' })}>Individual</button>
              <button className="heating-button" onClick={() => setFormData({ ...formData, heating: 'Combined' })}>Combined</button>
            </div>
          </div>
          ...
          <div className="class-filters">
            <label>Fill in all required filters<span className="necessarily">*</span></label>
  ...
  <div className='posting-an-ad'>
    ...
    <div className="photo-video-container">
      <div className="photo-pyramid">
        <div className="upload-photo" onClick={() => document.getElementById('photo-upload-1').click()}>
          {renderButtonContent('photo', 1)}
        </div>
        <div className="upload-photo" onClick={() => document.getElementById('photo-upload-2').click()}>
          {renderButtonContent('photo', 2)}
        </div>
        <div className="upload-photo" onClick={() => document.getElementById('photo-upload-3').click()}>
      ...
      <div className="video-pyramid">
        <div className="upload-video" onClick={() => document.getElementById('video-upload').click()}>
          {renderButtonContent('video', 1)}
        </div>
        <div className="upload-photo" onClick={() => document.getElementById('photo-upload-4').click()}>
          {renderButtonContent('photo', 4)}
        </div>
        <div className="upload-photo" onClick={() => document.getElementById('photo-upload-5').click()}>
